...
function lar( models ) {
	...
	var m;
	var n;
				function mappingTo (y) {
							return this.numericals + y;
						}


				for ( m = 1; m <= models.length; m++ ) {
					
					for ( n = 1; n <= models[m].V.length; n++ ) {
						console.log(models[m].V[n]);
						V.push( models[m].V[n] );
					};
					
					for ( n = 1; n <= models[m].F.length; n++ ) {
						console.log(models[m].F[n]);
						F.push( models[m].F[n].map(mappingTo) );
					};
					
					numericals = numericals + models[m].V.length;
				};
				return [V,F];
};
